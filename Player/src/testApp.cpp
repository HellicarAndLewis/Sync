...
void testApp::setup() {
    ...
    openNIPlayer.setup();
    openNIPlayer.start();
    openNIPlayer.setRegister(true); //to match video to plots
...
void testApp::update(){
    ...
    int numberOfTrackedUsers = openNIPlayer.getNumTrackedUsers();
    
    if( numberOfTrackedUsers > 0){
        
        ofxOpenNIUser user = openNIPlayer.getTrackedUser(0); //get the 0th
